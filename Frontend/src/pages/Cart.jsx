...
const Cart = () => {
  ...
  const removeItemFromCart = async (productId) => {
    try {
      const res = await del("/cart/remove", {
        data: { productId }
      });
      if (res?.data?.success) {
        dispatch(setCart(res.data.cart));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 py-2">
      ...
      <div className="py-4 px-4 sm:px-6 flex flex-col gap-3 rounded-md custom-shadow h-fit">
        ...
        <div className="flex flex-col gap-2">
          ...
          <button className='py-2 rounded-md mt-1 text-sm bg-pink-600 text-white hover:bg-pink-700 duration-300 cursor-pointer font-semibold'>
            PLACE ORDER
          </button>
